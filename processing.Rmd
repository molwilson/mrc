...
ggplot(corals, aes(x = reorder(species, pc.tot), y = pc.tot)) +
  ...
  coord_flip() +
  labs(x = "Coral species", y = "Mean percent cover") +
  theme_bw() +
...
ggplot(sediment, aes(x = sedimentation, y = pc)) +
  ...
  facet_wrap(vars(site), nrow = 2) +
  labs(x = "Sedimentation level", y = "Mean percent cover") +
  theme_bw() +
  theme(#axis.text.x = element_blank(),
        legend.position = "bottom",
        legend.title = element_blank())
...
Notes...
  ...
  - Add graph of total herbivores by site, by family?
  - Add graph of size distribution of S. viride, S. vetula, S. iserti
  - Add graph of average biomass by species (across all sites) for appendix
...
ggplot(fish.bm, aes(x = site, y = bm)) +
  ...
  annotate("text", x = 6.5, y = 1350, size = 3, hjust = 1, label=c('Average unfished biomass for the Caribbean')) +
  labs(x = "", y = "Total fish biomass (kg/ha)") +
  theme_bw()
```
...
Notes:
  ...
  - I can't really find regional averages for herbivores or scarids/acanthurids.... I could include a reference line for Bonaire as an unfished comparison? But I'm not sure if that is just super random?
...
fish.bm.spp.t.raw <- fish %>%
  filter(biomass > 0) %>%
  group_by(site, transect, species_code) %>%
  summarize(bm.spp.t = sum(biomass/120)) # g/m2
...
Notes:
  ...
  - Need to decide what my reference points are here - mean terminal phase sizes in Bonaire, or other unfished populations? Will look through literature more...
  - could also look at sex ratio as better indicator? (and compare with that Bermuda paper...
  
```{r, eval = F}
scarids <- fish %>%
  filter(species_code %in% c("stop", "rbp", "strp") & phase_code %in% c("i", "t")) 
...
ggplot(diadema, aes(x = site, y = density)) +
  ...
  theme_bw() +
  labs(y = "Diadema density (indv. m-2)", x = "") +
...
ggplot(rugosity, aes(x = site, y = rugosity)) +
  ...
  annotate("text", x = 6.5, y = 2.2, size = 3, hjust = 1, label=c('Lesser Antilles mean from 1970s')) +
  theme_bw() +
