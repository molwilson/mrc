...
benthic <- read_excel(here("data", "MRC_benthic_V5.xlsx"), sheet = "benthic") %>% clean_names() %>%
  ...
  mutate(benthic_class = case_when(category_code %in% c("LC") ~ "Live hard corals",
         ...
         direction = case_when (category_code %in% c("LC", "SLC") ~ "Hard corals",
                                category_code %in% c("CCA", "CCA_ND", "PEY") ~ "Facilitators",
...
- making sure species names are lowercase
...
ggplot(benthic.pc.class, aes(x = benthic_class, y = pc, fill = direction)) +
  geom_col() +
  scale_fill_manual(values = c("palegreen3", "pink1", "coral1")) +
  facet_grid(. ~ site) +
  theme_bw() +
...
# barplot with positive/negative categories
...
### Percent cover by species for hard corals
...
### Relative abundance of coral species - currently this is average percent cover across transects within a site

*Note: eventually I think we should color code these by shape*
```{r}
...
ggplot(corals, aes(x = species, y = pc, fill = genus)) +
  ...
  theme_bw()
```
### Coral stress by site and by species
